...
void ultrasonic_callback(const sensor_msgs::LaserScan& g_ul)
{
    ...
    if((int)g_ul.intensities[0] == 1)
    {
        ul = g_ul.ranges[0];
        reliablity =  g_ul.intensities[0];
    }
//    g_ul_data.ranges[0] = g_ul.ranges[0];
//    g_ul_data.intensities[0] =  g_ul.intensities[0];
...
int main(int argc, char *argv[])
{
    ...
    ultrasonic_sub = nh.subscribe("/guidance/ultrasonic", 10, ultrasonic_callback);
...
/*	if(drone->rc_channels.pitch > 150)
	    ...
	    for(int i = 0; i < 100; i ++)
            {
                if(i < 90)
                    drone->attitude_control(0x4B, 0.5, 0, 0, 0);
                else
                    drone->attitude_control(0x4B, 0, 0, 0, 0);
                usleep(20000);
            }
