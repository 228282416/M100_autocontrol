...
void findcircle(IplImage* img, CvMemStorage* storage, vector<CvBox2D> &vEllipse)
{
	...
	{ //多边形逼近  
		if (contours->total > 10)
		{
			...
			sigm /= N;
			//if (sigm <= 0.35 && result.size.height > 60 && result.size.height < 180 && result.size.width > 60 && result.size.width < 180){
				//if (sigm <= 0.2){
			if (result.size.height > 60 && result.size.width > 60){
...
CvBox2D detection(IplImage* img)
{
	...
	findcircle(img_hsv, storage, result);
